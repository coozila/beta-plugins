...
class TestNewsSearch:
    def mock_response(self, *args, **kwargs):
        # Mock Response of NewsAPI. We have result for beta in technology but not others,
        # whereas Cricket is present in Sports/Entertainment but not others
        if kwargs["q"] == "AI" and kwargs["category"] == "technology":
            return json.loads(
                """{"status":"ok","totalResults":1,"articles": [{"title": "beta"}]}"""
            )
    ...
    def test_news_search(self):
        ...
        actual_output_beta = self.NewsSearch.news_headlines_search_wrapper("AI")
        assert actual_output_beta.count(["beta"]) == 1
        assert actual_output_beta.count([]) == 5
